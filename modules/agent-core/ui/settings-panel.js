...
import { xbPrompt } from '../../draw/shared/xb-dialog.js';
...
export function createAgentSettingsPanel(deps = {}) {
    ...
    // 页内对话框（xb-dialog.js），不用 window.prompt：部分内嵌浏览器屏蔽原生对话框。取消返回 null。
    async function promptForPresetName(message, fallback = '', root = null) {
        const current = normalizePresetName(fallback || DEFAULT_PRESET_NAME);
        const next = await xbPrompt(message, current, { title: '预设名称', document: root?.ownerDocument || undefined });
        if (next === null) {return null;}
        return normalizePresetName(next);
    }

    async function createPresetFromForm(root) {
        const draft = syncConfigDraft(root);
        const nextPresetName = await promptForPresetName('输入新预设名称：', `${draft.currentPresetName || DEFAULT_PRESET_NAME} 副本`, root);
        if (nextPresetName === null) return;
        if (!nextPresetName) {
            showToast?.('预设名称不能为空');
            return;
        }
    ...
    async function renameCurrentPreset(root) {
        const draft = syncConfigDraft(root);
        const currentPresetName = normalizePresetName(draft.currentPresetName || state.config?.currentPresetName || DEFAULT_PRESET_NAME);
        const nextPresetName = await promptForPresetName('输入预设名称：', draft.presetDraftName || currentPresetName, root);
        if (nextPresetName === null) return;
        if (!nextPresetName) {
            showToast?.('预设名称不能为空');
            return;
