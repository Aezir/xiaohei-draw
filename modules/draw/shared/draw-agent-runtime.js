...
export const OPENAI_RESPONSES_UNSUPPORTED_HINT = '当前接口类型是 OpenAI Responses，大多数中转站不支持，请在 API 配置 → 场景 Agent 改用『OpenAI 兼容』，并重新填写地址/key/模型';

/**
 * 中转站（new-api 类）不支持 /v1/responses 时给中文指引。
 * 条件：provider 是 openai-responses，状态码 404/405/500（501 = Not Implemented 也算），且信息含 not implemented / not found / unsupported。
 ...
 */
function mapProviderError(error, abortScope, upstreamSignal, provider = '') {
    ...
    try {
        for (let attempt = 1; attempt <= MAX_SCENE_PLANNER_ATTEMPTS; attempt += 1) {
            ...
            try {
                ...
                const error = mapProviderError(rawError, abortScope, options.signal, providerConfig.provider);
