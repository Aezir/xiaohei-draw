...
.xb-img-slot, .xb-img-slot *, .xb-img-slot *::before, .xb-img-slot *::after { border-style:none !important; outline-style:none !important; box-shadow:none !important; }
.xb-img-slot img.xb-generated-img { max-width:min(400px, 80%); max-height:60vh; border-radius:9px; display:block; cursor:pointer; transition:opacity .2s; }
.xb-img-slot img.xb-generated-img:hover { opacity:.9; }
.xb-img-placeholder, .xb-img-loading { display:inline-flex; align-items:center; gap:6px; padding:8px 12px; border-radius:8px; background:#202020; color:#9b9b98; font-size:12px; }
.xb-img-placeholder i, .xb-img-loading i { font-size:15px; line-height:1; }
.xb-img-loading i.xb-img-spin { animation:xbImgSpin 1s linear infinite; }
@keyframes xbImgSpin { to { transform:rotate(360deg); } }
.xb-img-error { display:inline-flex; flex-direction:column; align-items:center; gap:6px; padding:10px 14px; border-radius:8px; background:#202020; color:#e5534b; font-size:12px; }
.xb-img-error i { font-size:16px; }
.xb-img-retry { display:inline-flex; align-items:center; gap:4px; min-height:28px; padding:4px 10px; border-radius:6px; background:#2a2a2a; color:#e6e6e4; font-size:12px; cursor:pointer; }
.xb-img-retry:hover { background:#333333; }
`;
...
function hydrateSlots(container) {
    container.querySelectorAll('[data-xb-draw-chat-image="1"]').forEach((slot) => {
        ...
        slot.innerHTML = '<div class="xb-img-placeholder"><i class="ri-image-line"></i><span>滚动加载</span></div>';
...
async function loadImage(slot, tags) {
    ...
    if (!isAvailable()) return;
    // eslint-disable-next-line no-unsanitized/property
    slot.innerHTML = '<div class="xb-img-loading"><i class="ri-loader-4-line xb-img-spin"></i> 检查缓存...</div>';
    const controller = new AbortController();
    activeControllers.add(controller);
    try {
        const generateSharedImage = window.xbdrawDraw?.generateSharedImage;
            ...
            onProgress(status, ahead) {
                ...
                if (status === 'queued') {
                    ...
                    slot.innerHTML = ahead > 0
                        ? `<div class="xb-img-loading"><i class="ri-time-line"></i> 前方 ${ahead} 张</div>`
                        : '<div class="xb-img-loading"><i class="ri-time-line"></i> 已进入队列</div>';
                } else if (status === 'generating') {
                    // eslint-disable-next-line no-unsanitized/property
                    slot.innerHTML = '<div class="xb-img-loading"><i class="ri-loader-4-line xb-img-spin"></i> 生成中...</div>';
                }
            },
        });
        if (!initialized || generation !== lifecycleGeneration || controller.signal.aborted) return;
        ...
        // eslint-disable-next-line no-unsanitized/property
        slot.innerHTML = `<div class="xb-img-error"><i class="ri-error-warning-line"></i><div>${escapeHtml(error?.message || '失败')}</div><button class="xb-img-retry" data-tags="${encodeURIComponent(tags)}"><i class="ri-refresh-line"></i>重试</button></div>`;
