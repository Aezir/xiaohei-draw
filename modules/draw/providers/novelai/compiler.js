...
import { getNovelModelCapability, isNovelV5Model, NOVEL_VIBE_MAX, supportsNovelVibe } from './novel-model-capabilities.js';
...
// V4/V4.5 发送已编码的 vibe token 时是否附带 reference_information_extracted_multiple。
// NekoAI（按网页客户端抓包校验）的做法是不发；第一次真实生成若报 400，把这里改成 true 再试（vibe-plan §7 第 1 条）。
export const NOVEL_VIBE_SEND_INFORMATION_EXTRACTED = false;
...
function applySizeOverride(params, overrideSize) {
    // 口径和设置页提示条 / 计价共用 novel-effective-size.js
    const override = parseOverrideSize(overrideSize);
    if (!override) return { ...params };
    return { ...params, width: override.width, height: override.height };
}

/**
 * 校验并整理已解析好的氛围（novel-vibe-resolve.js 的输出）。
 * 不支持氛围的模型（V3 / V5 / 其它）直接返回空数组 —— 请求里不会出现任何 reference_* 字段。
 */
export function normalizeCompiledNovelVibes(vibes, model) {
    if (!Array.isArray(vibes) || vibes.length === 0 || !supportsNovelVibe(model)) return [];
    if (vibes.length > NOVEL_VIBE_MAX) {
        throw new TypeError(`NovelAI 氛围最多 ${NOVEL_VIBE_MAX} 个`);
    }
    return vibes.map((vibe, index) => {
        const token = typeof vibe?.token === 'string' ? vibe.token.trim() : '';
        if (!token) throw new TypeError(`NovelAI 第 ${index + 1} 个氛围缺少编码 token（生成前必须先编码）`);
        const strength = Number(vibe.strength);
        if (!Number.isFinite(strength) || strength <= 0 || strength > 1) {
            throw new TypeError(`NovelAI 第 ${index + 1} 个氛围强度必须在 0～1 之间`);
        }
        const informationExtracted = Number(vibe.informationExtracted ?? 1);
        return { token, strength, informationExtracted: Number.isFinite(informationExtracted) ? informationExtracted : 1 };
    });
}
...
export function buildNovelAIRequestBody({
    ...
    seed,
    vibes = [],
    sendVibeInformationExtracted = NOVEL_VIBE_SEND_INFORMATION_EXTRACTED,
} = {}) {
    ...
    const compiledVibes = normalizeCompiledNovelVibes(vibes, modelName);
    if (compiledVibes.length > 0) {
        // V4/V4.5：reference_image_multiple 放编码后的 token（不是原图）。
        body.parameters.reference_image_multiple = compiledVibes.map(vibe => vibe.token);
        body.parameters.reference_strength_multiple = compiledVibes.map(vibe => vibe.strength);
        if (sendVibeInformationExtracted) {
            body.parameters.reference_information_extracted_multiple = compiledVibes.map(vibe => vibe.informationExtracted);
        }
    }
    return body;
}
...
export function compileNovelImageRequest(request, generationRecipe, seed) {
    ...
    return {
        ...
        payload: buildNovelAIRequestBody({
            ...
            params,
            seed,
            vibes: request?.vibes ?? recipe.vibes ?? [],
            sendVibeInformationExtracted: recipe.sendVibeInformationExtracted ?? NOVEL_VIBE_SEND_INFORMATION_EXTRACTED,
        }),
