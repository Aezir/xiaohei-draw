...
import {
    ...
    NAI_BACKEND_BASE,
    NAI_BACKEND_MIN_VERSION,
    resolveNovelAIBackendImageApi,
    snapshotNovelRequestConfig,
} from './novel-request-config.js';
import { normalizeNovelVibeConfig } from './novel-vibe-config.js';
import { getVibeStore } from './novel-vibe-store.js';
import { encodeVibe } from './novel-vibe-api.js';
import { readVibeAssetImageBase64 } from './novel-vibe-resolve.js';
import { bytesToBase64 } from './novel-vibe-format.js';
import { buildNovelGenerationSnapshot, ensureNovelVibesEncoded } from './novel-vibe-generation.js';
import { resolveNovelSubscription, subscriptionFromManualTier } from './novel-subscription.js';
import {
    loadTagGuide,
    loadPromptTemplates,
...
import {
    ...
    buildImageHtml,
    closeImageCardEditor,
    ensureChatImageStyles,
    openImageCardEditor,
    setImageCardState,
    updateImageCardHistory,
} from '../../shared/chat-image-card.js';
import { attachChatImageCardGestures } from '../../shared/chat-image-gestures.js';
import { downloadImageOriginal, registerLightboxAction } from '../../shared/image-lightbox.js';
import { destroyChatImageMenu, openChatImageMenu } from '../../shared/image-action-menu.js';
import { xbAlert, xbChoose, xbConfirm, xbPrompt } from '../../shared/xb-dialog.js';
import { applyNamedPresetImport, askPresetName, createPresetNameDialogs } from '../../shared/preset-naming.js';
import { presetGenerateConfirm } from './novel-cost-rule.js';
import { isSizeOverridden } from './novel-effective-size.js';
import { saveAndRegenerate } from './novel-edit-regenerate.js';
...
// 后端发送模式走 SillyTavern server plugin 转发（需安装 plugins/xbdraw-image-proxy 并开启 enableServerPlugins），
// 用于绕过浏览器 CORS / 自签证书限制。插件命名空间常量在 novel-request-config.js。
...
const NAI_BACKEND_V5_MIN_VERSION = NAI_BACKEND_MIN_VERSION;
...
async function assertV5BackendCapability(signal) {
    ...
    if (!status.ready) {
        ...
        throw new NovelDrawError(
            `NovelAI V5 后端插件${reason}，请安装当前 xbdraw-image-proxy（兼容后端最低 v${NAI_BACKEND_V5_MIN_VERSION}）`,
            ErrorType.NETWORK,
        );
    }
    if (!isVersionAtLeast(status.version, NAI_BACKEND_V5_MIN_VERSION)
        ...
        throw new NovelDrawError(
            `当前后端插件不支持 NovelAI V5 流协议，请安装当前 xbdraw-image-proxy（兼容后端最低 v${NAI_BACKEND_V5_MIN_VERSION}）`,
            ErrorType.NETWORK,
        );
    }
    return status;
...
function ensureStyles() {
    // 聊天图片卡的样式统一由 shared/chat-image-card.js 提供
    ensureChatImageStyles();
    if (document.getElementById('xbdraw-novel-draw-style')) return;
    const style = document.createElement('style');
    style.id = 'xbdraw-novel-draw-style';
    style.textContent = `
...
#xbdraw-novel-draw-overlay .nd-backdrop{position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7)}
#xbdraw-novel-draw-overlay .nd-frame-wrap{position:absolute;z-index:1}
#xbdraw-novel-draw-iframe{width:100%;height:100%;border:none;background:#191919}
@media(min-width:769px){#xbdraw-novel-draw-overlay .nd-frame-wrap{top:12px;left:12px;right:12px;bottom:12px}#xbdraw-novel-draw-iframe{border-radius:12px}}
@media(max-width:768px){#xbdraw-novel-draw-overlay .nd-frame-wrap{top:0;left:0;right:0;bottom:0}#xbdraw-novel-draw-iframe{border-radius:0}}
`;
    document.head.appendChild(style);
}

function syncOverlayHeight() {
    const overlay = document.getElementById('xbdraw-novel-draw-overlay');
    if (!overlay) return;
...
function syncOverlayFrameLayout() {
    const frameWrap = document.querySelector('#xbdraw-novel-draw-overlay .nd-frame-wrap');
    if (!frameWrap) return;
...
// reason：'user' 是用户亲手停的（停止键、Escape、面板取消）；其它是模块卸载、聊天切换这类生命周期中止。
...
function normalizeParamsPreset(preset, index) {
    ...
    return {
        ...
        vibes: normalizeNovelVibeConfig(source.vibes),
        // 从画廊导入的预设记住来源（查重用；V5 tag_hint 等原样放在 source.v5Hints）
        source: sanitizeGallerySource(source.source),
    };
}
...
function updateSettingsPersistent(mutator, okText = '已保存', options = {}) {
    // 任何入口（悬浮球 / 楼层菜单 / 快捷设置 / 设置页）改了尺寸覆盖，都同步给打开着的设置页（提示条 + 计价）。
    const update = async () => {
        const before = String(settingsCache?.overrideSize || 'default');
        const ok = await runSettingsPersistentUpdate(mutator, okText, options);
        const after = String(settingsCache?.overrideSize || 'default');
        if (ok && before !== after) postOverrideSizeToFrame(after);
        return ok;
    };
    const result = settingsUpdateQueue.then(update, update);
    settingsUpdateQueue = result.catch(() => {});
...
async function generateViaBackend({ url, legacyBaseUrl, apiKey, insecure, payload, signal, timeout }) {
    ...
    try {
        ...
        throw new NovelDrawError('后端代发失败（未安装 xbdraw-image-proxy 插件或 SillyTavern 未开启 server plugins）', ErrorType.NETWORK);
    }
    if (res.status === 404) {
        throw new NovelDrawError('后端端点不存在：请安装 plugins/xbdraw-image-proxy 并在 config.yaml 开启 enableServerPlugins 后重启酒馆', ErrorType.NETWORK);
    }
    if (!res.ok) {
...
async function generateV5ViaBackend({ url, apiKey, insecure, payload, signal, timeout }) {
    ...
    try {
        ...
        throw new NovelDrawError('V5 后端代发失败，请检查 xbdraw-image-proxy 插件', ErrorType.NETWORK);
    }
    if (response.status === 404) {
        throw new NovelDrawError(
            `V5 后端端点不存在：请安装当前 xbdraw-image-proxy（兼容后端最低 v${NAI_BACKEND_V5_MIN_VERSION}）`,
            ErrorType.NETWORK,
        );
    }
    if (!response.ok) {
...
async function testApiConnection(apiKey, baseUrl, opts = {}) {
    ...
    try {
        ...
        if (sendMode === 'backend') {
            ...
            if (res.status === 404) throw new NovelDrawError('后端端点不存在：请安装 plugins/xbdraw-image-proxy 并开启 enableServerPlugins 后重启酒馆', ErrorType.NETWORK);
            const data = await res.json().catch(() => null);
...
/**
 * V2：编译前确保氛围已编码（单张、批量/自动、前端/后端发送都走这里）。
 * 自动编码永远关（allowEncode:false）——编码只能由用户在氛围卡里亲手点「编码」。
 * 缺编码 / 编码失败 → 抛错，调用方整批中止；V3/V5 剥掉氛围并提示。
 */
async function ensureGenerationVibes(preset, model, { signal } = {}) {
    try {
        const result = await ensureNovelVibesEncoded({
            preset,
            model: String(model || preset?.params?.model || DEFAULT_PARAMS_PRESET.params.model).trim(),
            store: getVibeStore(),
            allowEncode: false,
            signal,
        });
        if (result.notices.length) showToast(result.notices.join('；'), 'info', 4000);
        return result;
    } catch (error) {
        if (signal?.aborted || error?.code === 'ABORTED') throw new NovelDrawError('已取消', ErrorType.ABORTED);
        throw new NovelDrawError(error?.message || String(error), ErrorType.REQUEST_CONFIG);
    }
}
...
async function runNovelImageBatch({
    ...
    const transportMode = requestConfig.sendMode;
...
/** 单张生成，返回 { base64, snapshot }（snapshot = 写进预览缓存的参数快照）。氛围取 preset（默认当前预设）。 */
async function generateNovelImageDetailed({ scene, characterPrompts, negativePrompt, params, generationConfig, signal, queueBatch, onQueueStateChange, preset = getActiveParamsPreset() }) {
    const vibeResult = await ensureGenerationVibes(preset, params?.model || preset?.params?.model, { signal });
    const request = { scene, characterPrompts, negativePrompt, params, vibes: vibeResult.vibes };
    let image = null;
    ...
    await runNovelImageBatch({
        ...
        onItemReady: ({ base64, prepared }) => { image = base64; preparedItem = prepared; },
        onItemSettled: ({ error }) => { failure = error; },
    });
    if (image) {
        return {
            base64: image,
            snapshot: buildNovelGenerationSnapshot({ prepared: preparedItem, request, vibeSummary: vibeResult.summary }),
        };
    }
    throw failure || new NovelDrawError('NovelAI 未返回图片', ErrorType.UNKNOWN);
}

async function generateNovelImage(options) {
    return (await generateNovelImageDetailed(options)).base64;
}
...
function setImageState(container, state) {
    setImageCardState(container, state);
}

function updateNavControls(container, currentIndex, total) {
    updateImageCardHistory(container, currentIndex, total);
}

// ═══════════════════════════════════════════════════════════════════════════
// 图片导航（手势绑定见 setupEventDelegation）
// ═══════════════════════════════════════════════════════════════════════════

// targetIndex：0 = 最新。更旧的一张（手指左往右）向右滑出，更新的一张（右往左）向左滑出。
async function navigateToImage(container, targetIndex) {
    const slotId = container.dataset.slotId;
    const currentIndex = parseInt(container.dataset.currentIndex) || 0;
    if (!slotId || container.dataset.navigating === '1' || targetIndex < 0 || targetIndex === currentIndex) return;
    ...
    container.dataset.navigating = '1';
    ...
    try {
        ...
        imgEl.src = getPreviewDisplayUrl(targetPreview);
        container.dataset.imgId = targetPreview.imgId;
        container.dataset.tags = targetPreview.tags || '';
        container.dataset.positive = targetPreview.positive || '';

        setImageState(container, targetPreview.savedUrl ? ImageState.SAVED : ImageState.PREVIEW);
        updateNavControls(container, targetIndex, successPreviews.length);
        ...
        imgEl.classList.add(`sliding-in-${direction}`);
        await new Promise(r => setTimeout(r, 230));
    } finally {
        imgEl.classList.remove('sliding-left', 'sliding-right', 'sliding-in-left', 'sliding-in-right');
        delete container.dataset.navigating;
    }
}
...
// 只剩占位卡按钮和编辑面板按钮走点击委托；真实图片卡的操作全部由手势触发。
async function handleDelegatedClick(e) {
    ...
    if (!action || !container.contains(actionEl)) return;
    ...
    switch (action) {
        case 'edit-tags':
            await toggleEditPanel(container, true);
            break;
        case 'save-tags':
            await saveTagsAndRegenerate(container);
            break;
        case 'cancel-edit':
            await toggleEditPanel(container, false);
            break;
        case 'retry-image':
...
function isImageCardBusy(card) {
    const state = card?.dataset?.state;
    return state === ImageState.SAVING || state === ImageState.REFRESHING || card?.dataset?.navigating === '1';
}

// 卡片当前这张图的预览记录：优先画廊缓存（有 base64 和参数快照），没有就用卡片当前的图片地址（savedUrl）。
async function resolveCardPreview(card) {
    const imgId = card.dataset.imgId;
    let preview = null;
    try { preview = imgId ? await getPreview(imgId) : null; } catch { }
    if (!preview?.base64 && !preview?.savedUrl) {
        const src = card.querySelector('.xb-nd-img-wrap > img')?.getAttribute('src') || '';
        preview = { imgId, slotId: card.dataset.slotId, savedUrl: src };
    }
    return preview;
}

// 长按面板「下载」：下载原图。
async function downloadCardOriginal(card, knownPreview = null) {
    const preview = knownPreview || await resolveCardPreview(card);
    try {
        const name = await downloadImageOriginal(preview);
        showToast(`已开始下载原图：${name}`, 'info');
    } catch (error) {
        console.error('[NovelDraw] 下载原图失败:', error);
...
async function openCardActionMenu(card, { x, y }) {
    ...
    const messageId = parseInt(card.dataset.mesid);
    openChatImageMenu(x, y, {
        slotId: card.dataset.slotId,
        messageId: Number.isInteger(messageId) ? messageId : null,
        preview,
        index: parseInt(card.dataset.currentIndex) || 0,
...
function setupEventDelegation() {
    ...
    chatImageGestures = attachChatImageCardGestures(document, {
        ...
        regenerate: (card) => { void refreshSingleImage(card); },
    });
}
...
async function toggleEditPanel(container, show) {
    if (!container?.querySelector('.xb-nd-edit')) return;
    if (!show) {
        closeImageCardEditor(container);
        return;
    }
    let characterPrompts = [];
    const imgId = container.dataset.imgId;
    if (imgId) {
        try { characterPrompts = (await getPreview(imgId))?.characterPrompts || []; } catch { }
    }
    openImageCardEditor(container, { tags: container.dataset.tags || '', characterPrompts });
}

async function saveEditedTags(container, { silent = false } = {}) {
    const imgId = container.dataset.imgId;
    const slotId = container.dataset.slotId;
    const messageId = parseInt(container.dataset.mesid);
    ...
    if (originalPreview) {
        ...
        await storePreview({
            ...
            params: originalPreview.params || null,
        });
...
// 计价用的档位：只读内存里已查到的结果（不为此发请求）；没查到 = null，估算为「约」，一律确认。
function currentPricingSubscription() {
    const result = subscriptionCache?.result;
    if (!result) return null;
    return applyManualTierFallback(result, normalizeSubTier(getRuntimeSettings().subTier))?.subscription || null;
}

function confirmEditRegenerate(reasons) {
    return xbConfirm(`- ${reasons.join('\n- ')}`, {
        title: '重新生成前确认',
        detail: '用的是当前参数预设。取消也会保留刚才保存的提示词。',
        okLabel: '仍要生成',
        icon: 'ri-coins-line',
    });
}

function editRegenerateDeps(container) {
    return {
        validate: () => (container.querySelector('.xb-nd-edit textarea[data-type="scene"]')?.value.trim() ? '' : '场景 TAG 不能为空'),
        check: () => presetGenerateConfirm(getActiveParamsPreset(), currentPricingSubscription(), getSettings().overrideSize),
        confirm: confirmEditRegenerate,
        alert: message => xbAlert(message),
        notify: message => showToast(message, 'info'),
    };
}

// 编辑提示词窗口「保存并重新生成」：保存 → 花费规则（需要时页内确认）→ 对该槽位重新生成一张（新图成为最新版本）。
async function saveTagsAndRegenerate(container) {
    await saveAndRegenerate(container, {
        ...editRegenerateDeps(container),
        save: () => saveEditedTags(container, { silent: true }),
        regenerate: () => refreshSingleImage(container),
    });
}
...
async function refreshSingleImage(container) {
    ...
    try {
        ...
        const stillAttached = currentContext.chatId === sourceContext.chatId
            && currentContext.chat?.[messageId] === sourceMessage;
        if (stillAttached) {
            void xbAlert('刷新失败: ' + e.message, { title: '刷新失败' });
            const activeContainer = getMesTextElement(messageId)?.querySelector(buildDrawSlotSelector(slotId));
...
async function retryFailedImage(container) {
    ...
    container.innerHTML = buildBusyInnerHtml('生成中');
...
async function removePlaceholder(container) {
    const slotId = container.dataset.slotId;
    const messageId = parseInt(container.dataset.mesid);
    if (!(await xbConfirm('确定移除此占位符？', { danger: true, okLabel: '移除' }))) return;
    await deleteFailedRecordsForSlot(slotId);
...
function notifyDetachedGeneration(successCount) {
    ...
    if (count > 0) {
        toastr.info(`聊天或楼层已经变化，已生成 ${count} 张图片但未写入原楼层；可在画图设置的图片管理中查看。`, '小黑生图');
    }
}
...
async function runGenerateImagesFromText(options = {}, drawLog = null) {
    ...
    try {
        ...
        // V2：编译前确保氛围已编码；缺编码或编码失败整批不开始
        const vibeResult = await ensureGenerationVibes(preset, preset.params?.model, { signal });
        ...
        const batchResult = await runNovelImageBatch({
            ...
            onItemReady: async ({ index, base64, prepared }) => {
                ...
                await storePreview({
                    ...
                    params: buildNovelGenerationSnapshot({ prepared, request: item.request, vibeSummary: vibeResult.summary }),
                });
...
async function runGenerateAndInsertImages({
    ...
    try {
        ...
        // V2：在动正文、插占位之前确保氛围已编码；缺编码或编码失败整批不开始
        const vibeResult = await ensureGenerationVibes(preset, preset.params?.model, { signal });
...
function createOverlay() {
    ...
    overlay.id = 'xbdraw-novel-draw-overlay';
    ...
    iframe.id = 'xbdraw-novel-draw-iframe';
    iframe.src = `${HTML_PATH}?v=${Date.now()}`;
    iframe.style.cssText = 'width:100%;height:100%;border:none;background:#191919;display:block;';
...
async function sendInitData() {
    ...
    postToIframe(iframe, buildPayload(), 'XBDraw-NovelDraw');
    ...
    const currentIframe = document.getElementById('xbdraw-novel-draw-iframe');
...
// ═══════════════════════════════════════════════════════════════════════════
// V2：订阅档位查询 + 氛围编码（设置页消息，集中放一处）
// ═══════════════════════════════════════════════════════════════════════════

const NOVEL_SUB_TIER_VALUES = ['auto', 'none', 'tablet', 'scroll', 'opus'];

function normalizeSubTier(value) {
    const tier = String(value || '').trim().toLowerCase();
    return NOVEL_SUB_TIER_VALUES.includes(tier) ? tier : 'auto';
}

function postToSettingsFrame(payload) {
    const iframe = document.getElementById('xbdraw-novel-draw-iframe');
    if (iframe) postToIframe(iframe, payload, 'XBDraw-NovelDraw');
}

function postOverrideSizeToFrame(overrideSize) {
    postToSettingsFrame({ type: 'OVERRIDE_SIZE', overrideSize: String(overrideSize || 'default') });
}

// 只在内存里缓存（按 Key 区分），不落盘；Key 不进日志。
let subscriptionCache = { apiKey: null, result: null, pending: null };

function applyManualTierFallback(result, manualTier) {
    if (result?.source === 'direct' || result?.source === 'plugin') return result;
...
async function refreshSubscriptionForFrame({ force = false } = {}) {
    ...
    let entry = subscriptionCache;
    if (force || entry.apiKey !== apiKey || (!entry.result && !entry.pending)) {
        const pending = resolveNovelSubscription({
            apiKey,
            manualTier: 'auto',
            ...
            source: 'unknown',
            subscription: null,
            errors: [{ code: error?.code || 'unknown', message: error?.message || String(error) }],
        }));
        entry = { apiKey, result: null, pending };
        subscriptionCache = entry;
        pending.then((result) => { entry.result = result; });
    }
    ...
    postToSettingsFrame({
        ...
        manualTier,
        errors: (view.errors || []).map(error => ({ code: String(error.code || ''), message: String(error.message || '').slice(0, 200) })),
    });
}

const vibeEncodeInFlight = new Set();

/** 氛围卡「编码」按钮（用户亲手点击才会到这里，每次 2 Anlas，不重试）。 */
async function handleVibeEncodeRequest(data) {
    const assetId = String(data?.assetId || '');
    const model = String(data?.model || '').trim();
    const informationExtracted = Number(data?.informationExtracted);
    const flightKey = `${assetId}|${model}|${informationExtracted}`;
    const reply = payload => postToSettingsFrame({
        type: 'VIBE_ENCODE_RESULT',
    ...
    if (vibeEncodeInFlight.has(flightKey)) {
        reply({ ok: false, error: '这个氛围正在编码中' });
        return;
    }
    vibeEncodeInFlight.add(flightKey);
    try {
        if (!assetId) throw new Error('缺少氛围图');
        if (!supportsNovelVibe(model)) throw new Error('氛围编码只支持 NovelAI V4 / V4.5 模型');
        const settings = getRuntimeSettings();
        if (!String(settings.apiKey || '').trim()) throw new Error('请先在 API 配置里填写 API Key');
        const store = getVibeStore();
        const cached = await store.getEncoding(assetId, model, informationExtracted);
        if (cached?.token) {
            reply({ ok: true, cached: true, bytes: cached.bytes || 0 });
            return;
        }
        ...
        console.log(`[NovelDraw] 氛围编码完成（${via}，${bytes.byteLength} bytes）`);
        reply({ ok: true, via, bytes: bytes.byteLength });
    } catch (error) {
        reply({ ok: false, error: error?.message || String(error) });
    } finally {
        vibeEncodeInFlight.delete(flightKey);
    }
}

function postStatus(state, text, target = '') {
    const iframe = document.getElementById('xbdraw-novel-draw-iframe');
    if (iframe) postToIframe(iframe, { type: 'STATUS', state, text, target }, 'XBDraw-NovelDraw');
}

// ═══════════════════════════════════════════════════════════════
// 画廊 G2：灯箱长按「存入画廊」+ 图片管理批量存入
// 目标：连了同步仓库存远端，没连存本机画廊（IndexedDB xb_gallery_local）。凭据只在 IndexedDB，不进设置文件、不走 postMessage。
// ═══════════════════════════════════════════════════════════════
let galleryLightboxOff = null;
let galleryStores = null;
let galleryBatchRunning = false;

function getGalleryStores() {
    if (!galleryStores) galleryStores = { credentials: createCredentialStore(), local: createLocalGallery() };
    return galleryStores;
}

/** 批次名要用「生成这张图的那条消息」的角色名（群聊）；只有预览属于当前聊天时才取消息 */
function resolveGalleryMessageContext(preview, messageId) {
    const ctx = getContext();
    const sameChat = !preview?.chatId || String(preview.chatId) === String(ctx?.chatId || '');
    const mid = Number(messageId ?? preview?.messageId);
    return { message: sameChat && Number.isInteger(mid) ? ctx?.chat?.[mid] : undefined, name2: ctx?.name2 };
}

function ensureGalleryLightboxAction() {
    if (galleryLightboxOff) return;
    try {
        galleryLightboxOff = registerGalleryLightboxAction(registerLightboxAction, {
            getTarget: createTargetResolver(getGalleryStores()),
            resolveContext: ctx => resolveGalleryMessageContext(ctx?.preview, ctx?.messageId),
            confirm: text => xbConfirm(text, { title: '同步到 Gallery', okLabel: '重新加回' }),
            notify: (text, kind) => showToast(text, kind === 'error' ? 'error' : kind === 'success' ? 'success' : 'info'),
        });
    } catch (e) {
...
async function runGalleryBatchExport(data = {}) {
    const iframe = document.getElementById('xbdraw-novel-draw-iframe');
    const reply = payload => { if (iframe) postToIframe(iframe, payload, 'XBDraw-NovelDraw'); };
    if (galleryBatchRunning) { postStatus('error', '正在存入画廊，等这一批完成再点', 'gallery'); return; }
    galleryBatchRunning = true;
    try {
        let previews = [];
        if (Array.isArray(data.imgIds) && data.imgIds.length) {
            previews = await Promise.all(data.imgIds.slice(0, 500).map(id => getPreview(String(id)).catch(() => null)));
        } else if (data.charName) {
            previews = Object.values(await getCharacterPreviews(String(data.charName))).flat();
        }
        previews = previews.filter(p => p && p.status !== 'failed' && (p.base64 || p.savedUrl));
        if (!previews.length) { postStatus('error', '没有可以存入画廊的图', 'gallery'); return; }
        const target = await createTargetResolver(getGalleryStores())();
        if (!target) { postStatus('error', '画廊还不能用：本机存储没有准备好', 'gallery'); return; }
        const where = target.kind === 'remote' ? '画廊' : '本机画廊';
        postStatus('loading', `存入${where}：0 / ${previews.length}`, 'gallery');
        const items = previews.map(preview => ({ preview, ...resolveGalleryMessageContext(preview) }));
        const result = await exportChatImagesBatch(items, {
            target,
            onProgress: p => postStatus('loading', p.phase === 'upload'
                ? `存入${where}：提交中（已处理 ${p.done} / ${p.total}）`
                : `存入${where}：${p.done} / ${p.total}`, 'gallery'),
        });
        const summary = describeBatchResult(result);
        postStatus(summary.kind === 'error' ? 'error' : 'success', summary.text, 'gallery');
        if (result.errors.length) console.warn('[NovelDraw] 存入画廊失败的图:', result.errors);
        reply({ type: GALLERY_HOST_REPLIES.BATCH_DONE, added: result.added.length, target: result.target });
    } catch (e) {
        postStatus('error', e?.message || '存入画廊失败', 'gallery');
    } finally {
...
function getAgentSettingsSurfaceRoot() {
    return document.getElementById('xbdraw-novel-draw-iframe')
        ?.contentDocument
...
async function handleFrameMessage(event) {
    ...
    switch (data.type) {
        case 'FRAME_READY':
            ...
            void refreshSubscriptionForFrame();
        ...
        case 'SAVE_FLOAT_FIELDS': {
            const floatFields = normalizeFloatFields(data.floatFields);
            const ok = await updateSettingsPersistent((settings) => {
                settings.floatFields = floatFields;
            }, '悬浮菜单已更新', { target: 'params' });
            if (ok) {
                try {
                    const fp = await import('./floating-panel.js');
                    fp.rebuildMenus?.();
        ...
        case 'SAVE_OVERRIDE_SIZE': {
            // 设置页提示条「取消覆盖」/ 复位：和悬浮球菜单改尺寸走同一个字段、同一个保存队列
            const value = String(data.overrideSize || 'default');
            if (value === 'default' || isSizeOverridden(value)) {
                const ok = await updateSettingsPersistent((settings) => {
                    settings.overrideSize = value;
                }, value === 'default' ? '已取消悬浮球尺寸覆盖' : '悬浮球尺寸已更新', { target: 'params' });
                if (ok) {
                    try {
                        const fp = await import('./floating-panel.js');
                        fp.refreshPresetSelectAll?.();
                    } catch {}
                }
            }
            // 无论成功与否都回发真实值，设置页的乐观更新以此为准
            postOverrideSizeToFrame(getSettings().overrideSize);
            break;
        }
        ...
        case 'SAVE_SUB_TIER': {
            const subTier = normalizeSubTier(data.subTier);
            await updateSettingsPersistent((settings) => {
                settings.subTier = subTier;
            }, '已保存', { notify: false, silent: true });
            void refreshSubscriptionForFrame();
            break;
        }

        case 'REFRESH_SUBSCRIPTION':
            void refreshSubscriptionForFrame({ force: data.force === true });
            break;

        case 'VIBE_ENCODE':
            await handleVibeEncodeRequest(data);
            break;
        ...
        case 'CHECK_BACKEND_PLUGIN': {
            ...
            const iframe = document.getElementById('xbdraw-novel-draw-iframe');
            if (iframe) postToIframe(iframe, { type: 'BACKEND_PLUGIN_STATUS', status }, 'XBDraw-NovelDraw');
            break;
        }
        ...
        // 画廊 G1b：从画廊记录导入参数预设。iframe 发来的消息只取预设字段（不含凭据），source 保留查重。
        case 'IMPORT_GALLERY_PRESET': {
            const iframe = document.getElementById('xbdraw-novel-draw-iframe');
            const reply = payload => { if (iframe) postToIframe(iframe, payload, 'XBDraw-NovelDraw'); };
            let result;
            try {
                result = applyGalleryPresetImport(getSettings().paramsPresets, data, {
                    normalize: normalizeParamsPreset,
                    makeId: () => `params-gallery-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
                });
            } catch (e) {
                reply({ type: GALLERY_HOST_REPLIES.IMPORTED, ok: false, error: e?.message || '导入失败' });
                break;
            }
            if (result.status === 'duplicate') {
                reply({ type: GALLERY_HOST_REPLIES.DUPLICATE, duplicateOf: result.duplicateOf });
                break;
            }
            if (result.status === 'name-conflict') {
                reply({ type: GALLERY_HOST_REPLIES.NAME_CONFLICT, name: result.name, nameConflictOf: result.nameConflictOf, duplicateOf: result.duplicateOf });
                break;
            }
            if (result.status === 'cancelled') break;
            const saved = result.presets.find(p => p.id === result.presetId);
            const ok = await updateSettingsPersistent((settings) => {
                settings.paramsPresets = result.presets;
                settings.selectedParamsPresetId = result.presetId;
            }, result.status === 'overwritten' ? '已覆盖预设' : '已导入预设', { target: 'params' });
            reply({ type: GALLERY_HOST_REPLIES.IMPORTED, ok, status: result.status, presetId: result.presetId, name: saved?.name || '', error: ok ? '' : '保存设置失败' });
            if (ok) {
                sendInitData();
                try {
                    const { refreshPresetSelect } = await import('./floating-panel.js');
        ...
        case 'SAVE_GALLERY_CHARACTER_TAG': {
            const iframe = document.getElementById('xbdraw-novel-draw-iframe');
            const reply = payload => { if (iframe) postToIframe(iframe, payload, 'XBDraw-NovelDraw'); };
            let tag;
            try { tag = characterTagFromGallery({ name: data.name, prompt: data.prompt, uc: data.uc }); }
            catch (e) { reply({ type: GALLERY_HOST_REPLIES.CHARACTER_SAVED, ok: false, error: e?.message || '保存失败' }); break; }
            const ok = await updateSettingsPersistent((settings) => {
                settings.characterTags = [...(settings.characterTags || []), tag];
            }, '已保存角色标签', { notify: false, silent: true });
            reply({ type: GALLERY_HOST_REPLIES.CHARACTER_SAVED, ok, name: tag.name, error: ok ? '' : '保存设置失败' });
            if (ok) sendInitData();
            break;
        }

        // 画廊 G2：「图片管理」里把缓存的聊天图批量存入画廊（按角色，或单张 imgIds）
        case 'GALLERY_BATCH_EXPORT':
            void runGalleryBatchExport(data);
            break;
        ...
        case 'EXPORT_CURRENT_PRESET': {
            ...
            if (await downloadPresetAsFile(preset)) postStatus('success', '已导出', 'params');
            break;
        }
        ...
        case 'DELETE_GALLERY_IMAGE': {
            try {
                ...
                {
                    const iframe = document.getElementById('xbdraw-novel-draw-iframe');
                    if (iframe) postToIframe(iframe, { type: 'GALLERY_IMAGE_DELETED', imgId: data.imgId }, 'XBDraw-NovelDraw');
                }
                sendInitData();
                showToast('已删除');
            } catch (e) {
        ...
        case 'TEST_SINGLE': {
            try {
                ...
                // 可选的测试 UC 接在负向固定之后；留空时与原快速测试完全一致
...
export async function initNovelDraw() {
    if (window?.isXbDrawEnabled === false) return;
    ...
    events.on(event_types.CHARACTER_MESSAGE_RENDERED, (data) => {
        ...
        if (Number(messageId) === findLastAIMessageId()) {
            floatingPanel.refreshGenerationUiState?.();
        }
...
export async function cleanupNovelDraw() {
    ...
    chatImageGestures?.destroy();
    chatImageGestures = null;
    destroyChatImageMenu();
    ...
    document.getElementById('xbdraw-novel-draw-overlay')?.remove();
    delete window.xbdrawNovelDraw;
