...
async function openUpstreamResponse({ url, key, payload, rawBody, contentType, insecure, signal, accept, method: initialMethod = 'POST' }) {
    let target = new URL(url);
    let method = initialMethod;
    let body = method === 'GET' ? null : (rawBody ?? JSON.stringify(payload));
...
function createMultipartRequest(payload) {
    const boundary = `----XBDrawImageProxy${randomBytes(12).toString('hex')}`;
...
async function generateV5ImageBuffer(options) {
    ...
    console.info(
        `[xbdraw-image-proxy] NovelAI V5 final arrived ${elapsedMs}ms after response opened; `
...
const MAX_VIBE_BYTES = 16 * 1024 * 1024;
const MAX_SUBSCRIPTION_BYTES = 1024 * 1024;

// POST /ai/encode-vibe：JSON {image, information_extracted, model}，上游返回原始二进制 vibe token。
// 每次调用扣 2 Anlas，这里只发一次，不重试。
async function encodeVibe({ url, key, payload, insecure, signal }) {
    const response = await openUpstreamResponse({
        url,
        key,
        payload,
    ...
    const body = await readResponseBuffer(response, signal, MAX_VIBE_BYTES);
    if (body.length === 0) throw new Error('NovelAI returned an empty vibe encoding');
    return { ok: true, base64: body.toString('base64'), bytes: body.length };
}

// GET /user/subscription：只读，不扣 Anlas。原样把 JSON 交回。
async function fetchSubscription({ url, key, insecure, signal }) {
    const response = await openUpstreamResponse({
        url,
        key,
        method: 'GET',
    ...
    const body = await readResponseBuffer(response, signal, MAX_SUBSCRIPTION_BYTES);
    let subscription;
    try {
        subscription = JSON.parse(body.toString('utf8'));
    } catch {
        throw new Error('NovelAI subscription response is not valid JSON');
    }
